import * as admin from 'firebase-admin';
const groundskeepersData = [
  {
    id: "gk-1",
    name: "Arthur Weasley",
    avatar: "https://placehold.co/100x100.png",
    email: "arthur.w@example.com",
    phone: "555-0301",
    experienceYears: 15,
    expertiseAreas: ["Pitch Preparation", "Outfield Maintenance", "Cricket Square Management"],
    assignedFields: [
      { fieldName: "Northwood Main Oval", location: "Northwood School", conditionNotes: "Excellent condition, well-drained." },
      { fieldName: "Knights Field B", location: "Northwood School", conditionNotes: "Good for practice sessions." },
    ],
    bio: "Arthur is a highly experienced groundskeeper with a passion for creating perfect playing surfaces. He has managed top-tier school cricket grounds for over a decade.",
    availability: "Full-time",
  },
  {
    id: "gk-2",
    name: "Bertha Jorkins",
    avatar: "https://placehold.co/100x100.png",
    email: "bertha.j@example.com",
    phone: "555-0302",
    experienceYears: 8,
    expertiseAreas: ["Irrigation Systems", "Pest Control", "General Turf Management"],
    assignedFields: [
      { fieldName: "Hilton College Main Field", location: "Hilton College" },
      { fieldName: "Weightman-Smith Oval", location: "Hilton College", conditionNotes: "Currently undergoing minor aeration." },
    ],
    bio: "Bertha specializes in sustainable turf management and advanced irrigation techniques, ensuring fields remain playable throughout the season.",
    availability: "Full-time",
  },
  {
    id: "gk-3",
    name: "Cedric Diggory",
    avatar: "https://placehold.co/100x100.png",
    email: "cedric.d@example.com",
    phone: "555-0303",
    experienceYears: 5,
    expertiseAreas: ["Outfield Mowing Patterns", "Soil Health", "Seasonal Field Renovation"],
    assignedFields: [
      { fieldName: "Kingsmead Stadium (Contract)", location: "Durban" },
      { fieldName: "Chatsworth Oval (Consultant)", location: "Chatsworth" },
    ],
    bio: "Cedric is known for his meticulous attention to detail in outfield presentation and seasonal field renovations. He also consults for several prominent clubs.",
    availability: "Contract",
  },
  {
    id: "gk-4",
    name: "Doris Crockford",
    avatar: "https://placehold.co/100x100.png",
    email: "doris.c@example.com",
    phone: "555-0304",
    experienceYears: 20,
    expertiseAreas: ["Head Groundskeeper", "Budget Management", "Team Leadership", "Major Tournament Preparation"],
    assignedFields: [
      { fieldName: "All Westville Boys' High Fields", location: "Westville Boys' High School" },
    ],
    bio: "Doris is a veteran Head Groundskeeper with two decades of experience managing multi-field school sports complexes and preparing grounds for major tournaments.",
    availability: "Full-time",
  },
 {
    id: "gk-5",
    name: "Ernie Macmillan",
    avatar: "https://placehold.co/100x100.png",
    email: "ernie.m@example.com",
    phone: "555-0305",
    experienceYears: 10,
    expertiseAreas: ["Turf Aeration", "Drainage Solutions", "Fertilization"],
    assignedFields: [
      { fieldName: "Hillcrest College Green", location: "Hillcrest College" },
    ],
    bio: "Ernie is an expert in turf health, focusing on aeration and drainage to ensure playable surfaces even after heavy rain.",
    availability: "Full-time",
  },
  {
    id: "gk-6",
    name: "Filius Flitwick",
    avatar: "https://placehold.co/100x100.png",
    email: "filius.f@example.com",
    phone: "555-0306",
    experienceYears: 7,
    expertiseAreas: ["Outfield Striping", "Boundary Ropes Management", "Scoreboard Operations"],
    assignedFields: [
      { fieldName: "City Stadium", location: "City Council" },
    ],
    bio: "Filius brings precision to outfield presentation and is skilled in managing all aspects of match day setup.",
    availability: "Part-time",
  },
  {
    id: "gk-7",
    name: "Gilderoy Lockhart",
    avatar: "https://placehold.co/100x100.png",
    email: "gilderoy.l@example.com",
    phone: "555-0307",
    experienceYears: 12,
    expertiseAreas: ["Pitch Rolling", "Cricket Net Maintenance", "Lawn Mower Calibration"],
    assignedFields: [
      { fieldName: "Michaelhouse Oval", location: "Michaelhouse School" },
    ],
    bio: "Gilderoy is renowned for his perfectly rolled pitches and meticulous maintenance of training facilities.\",",
    availability: "Full-time",
  },
  {
    id: "gk-8",
    name: "Horace Slughorn",
    avatar: "https://placehold.co/100x100.png",
    email: "horace.s@example.com",
    phone: "555-0308",
    experienceYears: 18,
    expertiseAreas: ["Chemical Application", "Disease and Pest Identification", "Soil Testing"],
    assignedFields: [
      { fieldName: "Academy Ground", location: "Panthers Academy" },
      { fieldName: "Training Pitches", location: "Panthers Academy" },
    ],
    bio: "Horace is a specialist in turf science, ensuring the health and resilience of the playing surface through expert chemical application and soil analysis.",
    availability: "Full-time",
  },
  {
    id: "gk-9",
    name: "Igor Karkaroff",
    avatar: "https://placehold.co/100x100.png",
    email: "igor.k@example.com",
    phone: "555-0309",
    experienceYears: 9,
    expertiseAreas: ["Turf Repair", "Winter Preparations", "Heavy Traffic Area Management"],
    assignedFields: [
      { fieldName: "DHS Memorial Ground", location: "DHS" },
      { fieldName: "Kearsney AH Smith Oval", location: "Kearsney College" },
      { fieldName: "Clifton Riverside", location: "Clifton School" },
      { fieldName: "Goldstones", location: "Maritzburg College" },
    ],
    bio: "Igor is skilled in repairing damaged turf and preparing fields for challenging weather conditions and heavy use.",
    availability: "Contract",
  },
];

import serviceAccount from '/home/user/studio/scrbrd-beta-2-firebase-adminsdk-fbsvc-4c0a94b7bc.json' assert { type: 'json' };

// Initialize Firebase Admin if not already done
let firebaseApp;
if (admin.apps.length === 0) {
  firebaseApp = admin.initializeApp({
    credential: admin.credential.cert(serviceAccount)
  });
} else {
  firebaseApp = admin.app();
  console.log("Firebase app already initialized, using existing app.");
}


const db = firebaseApp.firestore();


async function migrateGroundkeepers() {
  console.log("Starting groundskeeper data migration to Firestore...");
  console.log(`Found ${groundkeepersData.length} groundskeepers to migrate.`);

  const groundskeeperCollectionRef = db.collection('groundskeeper');

  for (const groundskeeper of groundskeepersData) {
    try {
      await groundskeeperCollectionRef.doc(groundskeeper.id).set(groundskeeper);
      console.log(`Successfully migrated groundskeeper: ${groundskeeper.name} (ID: ${groundskeeper.id})`);
    } catch (error) {
      console.error(`Error migrating groundskeeper ${groundskeeper.name} (ID: ${groundskeeper.id}):`, error);
    }
  }

  console.log("Groundskeeper data migration complete.");
  process.exit(0);
}

migrateGroundkeepers().catch(error => {
  console.error("migrateGroundkeepersScript.js failed:", error);
  process.exit(1);
});