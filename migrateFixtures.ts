...
let schoolNameToIdMapCache: Map<string, string> | null = null; // Cache for school name to ID mapping

async function getSchoolNameToIdMap(): Promise<Map<string, string> | null> {
  if (schoolNameToIdMapCache !== null) {
    return schoolNameToIdMapCache;
  }
  try {
    const snapshot = await db.collection('schools').get();
    schoolNameToIdMapCache = new Map<string, string>();
...
async function getTeamId(teamIdentifier: string): Promise<string | null> {
  ...
  if (directMatch) {
    ...
    const potentialMatches = Array.from(schoolMap.entries()).filter(
      ([schoolName, id]) => schoolName.includes(schoolIdentifier)
    );
