...
const SidebarMenuButton = React.forwardRef(({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, disabled, ...props }, ref) => {
    const Comp = asChild ? Slot : "button";
    const { isMobile, state } = useSidebar();
    const button = (<Comp ref={ref} data-sidebar="menu-button" data-size={size} data-active={isActive} className={cn(sidebarMenuButtonVariants({ variant, size }), disabled && 'opacity-50 cursor-not-allowed', // Add disabled styling
        className)} aria-disabled={disabled} // Add aria-disabled attribute
     {...props}/>);
    if (!tooltip) {
        return button;
    }
