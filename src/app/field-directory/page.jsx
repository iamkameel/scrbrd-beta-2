...
export default function FieldDirectoryPage() {
    ...
    const filteredFields = React.useMemo(() => {
        return allFields.filter(field => {
            ...
            return (field.fieldName.toLowerCase().includes(searchLower) ||
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="relative flex-grow">
              ...
              <Input placeholder="Search by field name, school, or location..." className="pl-10" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
            </div>
            {/* Future filter dropdowns can go here (e.g., by location) */}
          </div>

          {filteredFields.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredFields.map((field) => (<Card key={field.id} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
            ...
            </div>) : (<p className="text-center text-muted-foreground py-4">No fields found matching your search criteria.</p>)}
        </CardContent>
      </Card>
    </div>);
}
