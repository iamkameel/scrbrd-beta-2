...
export default function TeamDetailsPage() {
    ...
    const team = detailedTeamsData.find(t => t.id === teamIdString);
    const teamFixtures = team ? allFixtures.filter(fixture => fixture.teamAId === team.id || fixture.teamBId === team.id) : [];
    if (!team) {
    ...
    const StatBox = ({ icon: Icon, label, value, className }) => (<div className={cn("flex flex-col items-center justify-center p-3 bg-muted rounded-lg shadow-sm text-center", className)}>
    ...
    return (<div className="container mx-auto py-8 space-y-8">
      ...
      <Card className="overflow-hidden">
        <div className="relative h-48 md:h-64 bg-muted">
          <Image src={`https://placehold.co/1200x400`} // Placeholder for banner image
     alt={`${team.schoolId} ${team.teamName} banner`} fill style={{ objectFit: "cover" }} data-ai-hint="team banner sport"/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {team.squad.map((player) => (<Card key={player.id} className="hover:shadow-md transition-shadow">
                    ...
                    <CardContent className="pt-0">
                      {player.stats && (<div className="grid grid-cols-2 gap-2 mt-2 text-xs">
                      ...
                      {player.stats?.bestBatting && (<div className="mt-3 text-xs border-t pt-2">
                      ...
                      {player.stats?.bestBowling && (<div className="mt-2 text-xs border-t pt-2">
                           <div className="flex items-center gap-1 text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              {teamFixtures.length > 0 ? (<div className="space-y-4">
                  {teamFixtures.map((fixture) => {
                const matchResult = fixture.status === "Completed"
                    ? resultsData.find((r) => r.fixtureId === fixture.id)
                    : undefined;
                return (<div key={fixture.id} className="p-4 border rounded-lg bg-muted/50 shadow-sm space-y-3">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                          <p className="font-semibold text-md mb-1 sm:mb-0">{fixture.teamAId} vs {fixture.teamBId}</p>
                          <Badge variant={getStatusBadgeVariant(fixture.status)} className={cn("whitespace-nowrap mt-1 sm:mt-0", fixture.status === "Upcoming" && "bg-[hsl(var(--accent))] text-accent-foreground border-transparent", fixture.status === "Completed" && "bg-[hsl(120,60%,30%)] text-accent-foreground border-transparent", fixture.status === "Rain-Delay" && "bg-[hsl(var(--primary))] text-primary-foreground border-transparent opacity-80", fixture.status === "Play Suspended" && "bg-[hsl(var(--chart-3))] text-card-foreground border-transparent", fixture.status === "Match Abandoned" && "bg-[hsl(var(--secondary))] text-muted-foreground opacity-80 border-transparent")}>
                            {fixture.status}
                          </Badge>
                        </div>
                        ...
                        {matchResult && (<div className="text-sm space-y-1 pt-1">
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                          {(fixture.status === "Upcoming" || fixture.status === "Scheduled") && (<Button asChild variant="outline" size="sm">
                              <Link href={`/prematch-team/${fixture.id}`}>
                                <ClipboardList className="mr-2 h-4 w-4"/>
                                View Pre-Match Team
                              </Link>
                            </Button>)}
                          {fixture.status === "Completed" && (<Button asChild variant="outline" size="sm">
                ...
                </div>) : (<p className="text-sm text-muted-foreground">No fixtures found for this team.</p>)}
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {team.performanceStats.titles !== undefined && team.performanceStats.titles > 0 && (<div className="flex items-center pt-2">
