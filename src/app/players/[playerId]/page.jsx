...
import { ArrowLeft, BarChart3, Briefcase, Info, ShieldCheck, Star, Target, Zap, Brain, CalendarDays, Hash, AtSign, Activity, TrendingUp, Crosshair, Users, ShieldQuestion, Loader2, AlertTriangle } from "lucide-react";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { format } from 'date-fns';
const StatItem = ({ label, value, icon: Icon }) => (<div className="flex justify-between py-2 border-b border-border/50 last:border-b-0">
...
const DetailedStat = ({ label, scoreDetail, icon: Icon }) => {
    if (!scoreDetail || !scoreDetail.value) {
        return (<div className="mt-4 pt-3 border-t">
    ...
    return (<div className="mt-4 pt-3 border-t">
      ...
      {scoreDetail.opponent && scoreDetail.year && scoreDetail.venue && (<p className="text-xs text-muted-foreground mt-0.5">
...
const PlayerStatDisplay = ({ label, value }) => (<div className="text-center">
...
const calculateOverallRating = (skills) => {
    ...
    let totalScore = 0;
    let skillCount = 0;
    const processSkillCategory = (category) => {
        if (category) {
...
export default function PlayerProfilePage() {
    ...
    return (<div className="container mx-auto py-8 space-y-6 print:space-y-0">
      ...
      <Card className="overflow-hidden shadow-xl bg-card border-2 border-primary/20 rounded-xl">
        ...
        <div className="p-6 bg-card rounded-b-lg grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 auto-rows-fr">
          ...
          {(player.stats.hundreds !== undefined || player.stats.fifties !== undefined) && (<div className="col-span-1 p-4 bg-accent/10 rounded-lg border border-border flex items-center justify-around">
              <PlayerStatDisplay label="100s" value={player.stats.hundreds}/>
              <PlayerStatDisplay label="50s" value={player.stats.fifties}/>
            </div>)}
          {(player.stats.catches !== undefined || player.stats.stumpings !== undefined) && (<div className="col-span-1 p-4 bg-accent/10 rounded-lg border border-border flex items-center justify-around">
              <PlayerStatDisplay label="Catches" value={player.stats.catches}/>
              <PlayerStatDisplay label="Stumpings" value={player.stats.stumpings}/>
            </div>)}
          {(player.stats.wickets !== undefined || player.stats.bowlingAverage !== undefined || player.stats.economyRate !== undefined) && (<div className="col-span-1 sm:col-span-2 lg:col-span-2 p-4 bg-accent/10 rounded-lg border border-border flex items-center justify-around">
