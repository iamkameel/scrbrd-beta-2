...
import {
  User, Search, Filter as FilterIcon, TrendingUp, Users as UsersIcon, Shield, MoreHorizontal, Loader2, AlertTriangle
} from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
...
// Minimal PlayerStats for CompactStatDisplay
interface CompactPlayerStats {
  matchesPlayed?: number;
  runs?: number;
  wickets?: number;
  average?: number | string; // Batting average
  bowlingAverage?: number | string; // Bowling average
}

// CompactStatDisplay component for displaying a single stat neatly
const CompactStatDisplay: React.FC<{ label: string; value: string | number | undefined; icon?: React.ElementType }> = ({ label, value, icon: Icon }) => (
  <div className="flex flex-col items-center text-center p-1 bg-muted/50 rounded-md flex-1 min-w-[70px]">
