...
import { Search, Filter as FilterIcon, UserCircle, Briefcase, CalendarClock, FilePenLine } from "lucide-react";
...
export default function ScorerProfilesPage() {
  ...
  const [levelFilter, setLevelFilter] = React.useState<ScorerProfile["certificationLevel"] | "all">("all");
  const [availabilityFilter, setAvailabilityFilter] = React.useState<ScorerProfile["availability"] | "all">("all");

  const uniqueLevels = React.useMemo(() => {
    const levels = new Set(scorersData.map(scorer => scorer.certificationLevel));
    return ["all", ...Array.from(levels).sort((a,b) => { // Custom sort for levels
      const order: Record<string, number> = { "Club Scorer": 0, "Level 1": 1, "Level 2": 2, "Advanced": 3, "all": -1 };
      return (order[a] || 99) - (order[b] || 99);
    })] as Array<ScorerProfile["certificationLevel"] | "all">;
  }, []);

  const uniqueAvailabilities = React.useMemo(() => {
    const availabilities = new Set(scorersData.map(scorer => scorer.availability));
    return ["all", ...Array.from(availabilities).sort()] as Array<ScorerProfile["availability"] | "all">;
  }, []);

  const filteredScorers = React.useMemo(() => {
    return scorersData.filter(scorer => {
      const searchLower = searchTerm.toLowerCase();
      const matchesSearch =
        searchTerm === "" ||
        scorer.name.toLowerCase().includes(searchLower) ||
        (scorer.associatedSchoolOrClub && scorer.associatedSchoolOrClub.toLowerCase().includes(searchLower));

      const matchesLevel =
        levelFilter === "all" || scorer.certificationLevel === levelFilter;
      ...
      <Card>
        ...
        <CardContent>
            ...
            <div className="flex gap-2 flex-wrap sm:flex-nowrap">
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuRadioGroup value={levelFilter} onValueChange={setLevelFilter as (value: string) => void}>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuRadioGroup value={availabilityFilter} onValueChange={setAvailabilityFilter as (value: string) => void}>
          ...
          {filteredScorers.length > 0 ? (
                ...
                <Card key={scorer.id} className="hover:shadow-lg transition-shadow flex flex-col">
                  <CardHeader className="flex flex-row items-start gap-4 pb-3">
                    ...
                    <div className="flex-1">
                      ...
                      <CardDescription>
                        <Badge variant="secondary" className="mr-1.5">{scorer.certificationLevel}</Badge>
                  ...
                  <CardContent className="flex-grow py-3 space-y-2">
                     <div className="grid grid-cols-2 gap-2">
                        ...
                        <CompactStatDisplay label="Matches" value={scorer.assignedMatchesCount} />
                     </div>
                     {scorer.associatedSchoolOrClub && (
                        <p className="text-sm text-muted-foreground pt-1">Affiliation: {scorer.associatedSchoolOrClub}</p>
                     )}
                  </CardContent>
            ...
            <p className="text-center text-muted-foreground py-4">No scorers found matching your search or filters.</p>
          )}
        </CardContent>
      </Card>
    </div>
