...
import { CalendarDays, Clock, MapPin, ListChecks, AlertTriangle, Loader2, Users, Info, LayoutGrid, List as ListIcon, MoreHorizontal, Edit3, FileText, Trash2 } from "lucide-react";
...
const getStatusBadgeVariant = (status, dateStr) => {
...
const getStatusDisplayName = (status, dateStr) => {
    const fixtureDate = parseISO(dateStr);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const fiveDaysFromNow = subDays(new Date(), -5);
...
const FixtureCard = ({ fixture, onAttemptDelete }) => {
    ...
    return (<Card className="hover:shadow-md transition-shadow">
      <CardHeader className="pb-3">
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center gap-1 ml-2 flex-shrink-0">
            <Badge variant={badgeVariant} className={cn("whitespace-nowrap", currentStatus === "Upcoming" && "bg-[hsl(var(--accent))] text-accent-foreground border-transparent", fixture.status === "Completed" && "bg-green-600 text-white border-transparent", fixture.status === "Live" && "bg-destructive text-destructive-foreground border-transparent animate-pulse", (fixture.status === "Rain-Delay" || fixture.status === "Play Suspended") && "bg-yellow-500 text-black border-transparent", fixture.status === "Match Abandoned" && "bg-gray-500 text-white opacity-80 border-transparent")}>
              {currentStatus}
            </Badge>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {(fixture.status === "Completed" || fixture.status === "Match Abandoned") && (<DropdownMenuItem asChild>
                    <Link href={`/scorecard/${fixture.id}`}>
                      <FileText className="mr-2 h-4 w-4"/>
                      View Scorecard
                    </Link>
                  </DropdownMenuItem>)}
                {(currentStatus === "Upcoming" || fixture.status === "Scheduled") && (<DropdownMenuItem asChild>
                    <Link href={`/prematch-team/${fixture.id}`}>
                      <Users className="mr-2 h-4 w-4"/>
                      Manage Pre-Match
                    </Link>
                  </DropdownMenuItem>)}
                <DropdownMenuItem className="text-destructive focus:bg-destructive/10 focus:text-destructive" onClick={() => onAttemptDelete(fixture.id)}>
      ...
      <CardContent className="space-y-2 text-sm">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-4 gap-y-2 text-muted-foreground">
          ...
          {(fixture.umpiresDisplay && fixture.umpiresDisplay !== 'N/A') || (fixture.scorerName && fixture.scorerName !== 'N/A') ? (<div className="pt-2 border-t mt-2 text-xs text-muted-foreground space-y-1">
            {fixture.umpiresDisplay && fixture.umpiresDisplay !== 'N/A' && (<div className="flex items-center">
                <Users className="mr-2 h-3.5 w-3.5 text-primary/80"/>
                <span>Umpires: {fixture.umpiresDisplay}</span>
              </div>)}
            {fixture.scorerName && fixture.scorerName !== 'N/A' && (<div className="flex items-center">
...
export default function FixturesPage() {
    const { data: fixtures, isLoading, isError, error } = useQuery({
        queryKey: ['fixtures'],
    ...
    const fixturesForSelectedDate = React.useMemo(() => {
        if (!selectedCalendarDate || !fixtures)
            return [];
    ...
    const [fixtureToDelete, setFixtureToDelete] = React.useState(null);
    const handleDeleteFixture = async (fixtureIdToDelete) => {
        ...
        <CardContent>
          <Tabs defaultValue="card" className="w-full">
            ...
            <TabsContent value="card">
              {fixtures && fixtures.length > 0 ? (<div className="space-y-4">
                  {fixtures.map((fixture) => (<FixtureCard key={`card-${fixture.id}`} fixture={fixture} onAttemptDelete={setFixtureToDelete}/>))}
                </div>) : (<p className="text-muted-foreground text-center py-4">No fixtures found for card view.</p>)}
            </TabsContent>

            <TabsContent value="list">
              {fixtures && fixtures.length > 0 ? (<ScrollArea className="h-[600px] border rounded-md">
                  <Table>
                ...
                const currentStatus = getStatusDisplayName(fixture.status, fixture.date);
                const badgeVariant = getStatusBadgeVariant(fixture.status, fixture.date);
                return (<TableRow key={`list-${fixture.id}`}>
                            ...
                            <TableCell className="text-right">
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  ...
                                  <DropdownMenuItem onClick={() => setFixtureToDelete(fixture.id)} className="text-destructive focus:bg-destructive/10 focus:text-destructive">
                                    <Trash2 className="mr-2 h-4 w-4"/>
                                    Delete Fixture
                                  </DropdownMenuItem>
                                </DropdownMenuContent>
                ...
                </ScrollArea>) : (<p className="text-muted-foreground text-center py-4">No fixtures found for list view.</p>)}
            </TabsContent>

            <TabsContent value="calendar">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="md:w-1/2 lg:w-2/5 flex justify-center md:justify-start">
                  <Calendar mode="single" selected={selectedCalendarDate} onSelect={setSelectedCalendarDate} modifiers={calendarModifiers} modifiersStyles={calendarModifiersStyles} className="rounded-md border shadow-sm p-2" initialFocus/>
                </div>
                <div className="md:w-1/2 lg:w-3/5">
                  <h4 className="text-lg font-semibold mb-2">
                    Fixtures for: {selectedCalendarDate ? format(selectedCalendarDate, 'PPP') : 'No date selected'}
                  </h4>
                  {fixturesForSelectedDate.length > 0 ? (<ScrollArea className="h-[400px] pr-3">
                      <div className="space-y-3">
                        {fixturesForSelectedDate.map((fixture) => (<FixtureCard key={`calendar-${fixture.id}`} fixture={fixture} onAttemptDelete={setFixtureToDelete}/>))}
                      </div>
                    </ScrollArea>) : (<p className="text-muted-foreground py-4">
          ...
          {(!fixtures || fixtures.length === 0) && (<p className="text-muted-foreground text-center py-4 mt-4">No fixtures found in the database. Try creating some!</p>)}
        </CardContent>
      </Card>
