...
import { detailedTeamsData as allTeams } from '@/lib/team-data';
import { scorersData as allScorersData } from '@/lib/scorer-data';
import { umpiresData as allUmpiresData } from '@/lib/umpire-data';
...
import { DropdownMenu, DropdownMenuCheckboxItem, DropdownMenuContent, DropdownMenuLabel as DropdownMenuLabelComponent, DropdownMenuSeparator, DropdownMenuTrigger, } from "@/components/ui/dropdown-menu";
import { Separator } from '@/components/ui/separator';
import { Loader2, Users, FilePenLine, CalendarDays, Clock, MapPin } from 'lucide-react';
const AGE_DIVISIONS = ["Open", "U16", "U15", "U14", "U13"];
const OPEN_CLASSES = ["1st XI", "2nd XI", "3rd XI", "4th XI", "5th XI", "Club Premier", "Club Reserve"];
const AGE_SPECIFIC_CLASSES = ["A", "B", "C", "D", "E", "F", "G", "H"];
...
export default function CreateFixturePage() {
    ...
    const [currentScorers] = useState(allScorersData);
    const [currentUmpires] = useState(allUmpiresData);
    ...
    useEffect(() => {
        if (watchHomeTeamSchoolId && watchAgeDivision) {
            ...
            if (selectedSchool) {
                ...
                const schoolTeams = allTeams.filter(team => String(team.schoolId) === String(selectedSchool.id) && team.ageGroup === watchAgeDivision && (watchAgeDivision === "Open" ? team.division === watchOpenClass : team.division === watchAgeSpecificClass));
    ...
    return (<Card className="w-full max-w-3xl mx-auto my-4 shadow-lg">
      <CardHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField control={form.control} name="matchType" render={({ field }) => (<FormItem>
                ...
                <FormField control={form.control} name="overs" render={({ field }) => (<FormItem>
                      <FormLabel>Overs {watchMatchType !== 'Test' ? '*' : '(Optional)'}</FormLabel>
                      <FormControl>
                        <Input type="number" {...field} value={field.value ?? ''} onChange={e => field.onChange(e.target.value === '' ? undefined : parseInt(e.target.value, 10))} readOnly={watchMatchType === 'T20' || watchMatchType === 'ODI'} className={(watchMatchType === 'T20' || watchMatchType === 'ODI') ? "bg-muted/50" : ""} placeholder={watchMatchType === 'Test' ? "e.g., 90" : "Enter overs"}/>
                      </FormControl>
                      <FormMessage />
                    </FormItem>)}/>
              </div>
            ...
            <div>
              ...
              <div className="space-y-4 border p-4 rounded-md bg-muted/20 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                    <FormField control={form.control} name="ageDivision" render={({ field }) => (<FormItem>
                          ...
                          <Select onValueChange={field.onChange} value={field.value || ''}>
                            ...
                            <SelectContent>
                              <SelectGroup>
                                {AGE_DIVISIONS.map((ag) => (<SelectItem key={ag} value={ag}>
                                    {ag}
                                  </SelectItem>))}
                              </SelectGroup>
                            </SelectContent>
                  ...
                  {watchAgeDivision === "Open" ? (<FormField control={form.control} name="openClass" render={({ field }) => (<FormItem>
                          ...
                          <Select onValueChange={field.onChange} value={field.value || ''}>
                            ...
                            <SelectContent>
                              <SelectGroup>
                                {OPEN_CLASSES.map((oc) => (<SelectItem key={oc} value={oc}>
                                    {oc}
                                  </SelectItem>))}
                              </SelectGroup>
                            </SelectContent>
                          ...
                          <FormMessage />
                        </FormItem>)}/>) : // Check if watchAgeDivision is one of the U-age divisions (implicitly not "Open")
            watchAgeDivision && AGE_DIVISIONS.slice(1).includes(watchAgeDivision) ? (<FormField control={form.control} name="ageSpecificClass" render={({ field }) => (<FormItem>
                          ...
                          <Select onValueChange={field.onChange} value={field.value || ''}>
                            ...
                            <SelectContent>
                              <SelectGroup>
                                {AGE_SPECIFIC_CLASSES.map((asc) => (<SelectItem key={asc} value={asc}>
                                    {asc}
                                  </SelectItem>))}
                              </SelectGroup>
                            </SelectContent>
                          ...
                          <FormMessage />
                        </FormItem>)}/>) : (<div className="text-sm text-muted-foreground h-10 flex items-center pt-7 md:col-span-1">Select Age Division first.</div>)}
                </div>
                <FormDescription>Select the overall age and class for this fixture. Teams will be filtered based on this.</FormDescription>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField control={form.control} name="homeTeamSchoolId" render={({ field }) => (<FormItem>
                      ...
                      <Select onValueChange={field.onChange} value={field.value || ''} disabled={!watchAgeDivision || (!watchOpenClass && !watchAgeSpecificClass)}>
                        ...
                        <SelectContent>
                          <SelectGroup>
                          {allSchools.map((school) => (<SelectItem key={String(school.id)} value={String(school.id)}>
                              {school.name}
                            </SelectItem>))}
                          </SelectGroup>
                        </SelectContent>
                ...
                <FormField control={form.control} name="homeTeamId" render={({ field }) => (<FormItem>
                      ...
                      <Select onValueChange={field.onChange} value={field.value || ''} disabled={!watchHomeTeamSchoolId || availableHomeTeams.length === 0}>
                        ...
                        <SelectContent>
                          <SelectGroup>
                          {availableHomeTeams.map((team) => (<SelectItem key={team.id} value={team.id}>
                 ...
                 <FormField control={form.control} name="awayTeamSchoolId" render={({ field }) => (<FormItem>
                ...
                <FormField control={form.control} name="awayTeamId" render={({ field }) => (<FormItem>
                      ...
                      <Select onValueChange={field.onChange} value={field.value || ''} disabled={!watchAwayTeamSchoolId || availableAwayTeams.length === 0}>
                        ...
                        <SelectContent>
                          <SelectGroup>
                          {availableAwayTeams.map((team) => (<SelectItem key={team.id} value={team.id}>
                              {team.teamName} ({team.ageGroup} {team.division})
                            </SelectItem>))}
                          </SelectGroup>
                        </SelectContent>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField control={form.control} name="scheduledDate" render={({ field }) => (<FormItem className="flex flex-col">
                      <FormLabel className="flex items-center"><CalendarDays className="mr-2 h-4 w-4 text-muted-foreground"/>Date *</FormLabel>
                      <FormControl>
                        <DatePicker selected={field.value} onChange={(date) => field.onChange(date)} dateFormat="yyyy/MM/dd" minDate={new Date()} className="w-full border rounded-md px-3 py-2 text-sm h-10" placeholderText="Select fixture date" autoComplete="off"/>
                      </FormControl>
                      <FormMessage />
                    </FormItem>)}/>
                ...
                <FormField control={form.control} name="venueId" render={({ field }) => (<FormItem className="md:col-span-2">
                      ...
                      <Select onValueChange={field.onChange} value={field.value || ''} disabled={!watchHomeTeamSchoolId || availableFields.length === 0}>
                        ...
                        <SelectContent>
                          <SelectGroup>
                          {availableFields.filter(fld => fld && fld.trim() !== '').map((fld) => (<SelectItem key={fld} value={fld}>
                              {fld}
                            </SelectItem>))}
                          </SelectGroup>
                        </SelectContent>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField control={form.control} name="umpireIds" render={({ field }) => (<FormItem>
                ...
                : "Select umpires"}
                        ...
                        <DropdownMenuContent className="w-full max-h-60 overflow-y-auto">
                          ...
                          {currentUmpires.map((umpire) => (<DropdownMenuCheckboxItem key={umpire.id} checked={field.value?.includes(umpire.id)} onCheckedChange={(checked) => {
                    return checked
                        ? field.onChange([...(field.value || []), umpire.id])
                        : field.onChange((field.value || []).filter((id) => id !== umpire.id));
                }}>
                ...
                <FormField control={form.control} name="scorerId" render={({ field }) => (<FormItem>
                      <FormLabel>Scorer (Optional)</FormLabel>
                      <Select onValueChange={field.onChange} value={field.value || 'no-scorer'}>
                        <FormControl>
                          <SelectTrigger>
                        ...
                        <SelectContent>
                          <SelectGroup>
                            ...
                            {currentScorers.map((scorer) => (<SelectItem key={scorer.id} value={scorer.id}>
                 ...
                 <FormField control={form.control} name="leagueId" render={({ field }) => (<FormItem>
                ...
                <FormField control={form.control} name="provinceId" render={({ field }) => (<FormItem>
