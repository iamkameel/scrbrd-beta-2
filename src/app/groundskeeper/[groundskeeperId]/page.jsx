...
import { ArrowLeft, Mail, Phone, Briefcase, CalendarClock, CheckCircle, MapPin, Sprout, Info } from "lucide-react"; // Added Sprout for expertise
...
export default function GroundskeeperDetailPage() {
    ...
    const groundskeeperId = params.groundskeeperId;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              {groundskeeper.assignedFields.length > 0 ? (<div className="space-y-4">
                  {groundskeeper.assignedFields.map((field, index) => (<Card key={index} className="bg-muted/30 p-4 shadow-sm">
                      <p className="font-semibold text-md">{field.fieldName}</p>
                      <p className="text-sm text-muted-foreground">Location: {field.location}</p>
                      {field.conditionNotes && (<p className="text-xs text-primary mt-1">Notes: {field.conditionNotes}</p>)}
                    </Card>))}
                </div>) : (<p className="text-muted-foreground">No specific fields currently assigned or listed.</p>)}
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="divide-y divide-border">
              ...
              <DetailItem label="Expertise Areas" icon={Sprout} value={groundskeeper.expertiseAreas.length > 0 ? (<ul className="list-disc list-inside space-y-1 mt-1">
