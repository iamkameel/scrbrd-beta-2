...
import Image from 'next/image'; // Keep if you decide to use next/image for avatars in listing
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Search, Filter as FilterIcon, UserCheck, Briefcase, CalendarClock, ShieldHalf } from "lucide-react"; // Changed UserCircle to UserCheck
...
export default function UmpireProfilesPage() {
  ...
  const [levelFilter, setLevelFilter] = React.useState<UmpireProfile["umpiringLevel"] | "all">("all");
  const [availabilityFilter, setAvailabilityFilter] = React.useState<UmpireProfile["availability"] | "all">("all");
  ...
  const uniqueLevels = React.useMemo(() => {
    ...
    return ["all", ...Array.from(levels).sort((a,b) => { // Custom sort for levels
      const order: Record<string, number> = { "Club Panel": 0, "Level 1": 1, "Level 2": 2, "National Panel": 3, "all": -1 };
      return (order[a] || 99) - (order[b] || 99);
    })] as Array<UmpireProfile["umpiringLevel"] | "all">;
  }, []);

  const uniqueAvailabilities = React.useMemo(() => {
    const availabilities = new Set(umpiresData.map(umpire => umpire.availability));
    return ["all", ...Array.from(availabilities).sort()] as Array<UmpireProfile["availability"] | "all">;
  }, []);
      ...
      <Card>
        ...
        <CardContent>
          ...
          {filteredUmpires.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground py-4">No umpires found matching your search or filters.</p>
          )}
        </CardContent>
      </Card>
    </div>
