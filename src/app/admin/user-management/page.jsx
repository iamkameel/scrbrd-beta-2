...
import { schoolsData } from '@/lib/schools-data'; // Import schoolsData
const ALL_ROLES = [
    "Admin", "School", "Coach", "Player", "Parent", "Captain",
    "Spectator", "Scorer", "Umpire", "Groundskeeper", "Sportmaster", "Driver"
];
const ALL_ROLES_FOR_FILTER = ["All", ...ALL_ROLES];
...
export default function UserManagementPage() {
    const [users, setUsers] = React.useState(initialSampleUsers);
    const [searchTerm, setSearchTerm] = React.useState("");
    const [schoolFilter, setSchoolFilter] = React.useState("All"); // Use schoolId for filter
    const [roleFilter, setRoleFilter] = React.useState("All");
    const [statusFilter, setStatusFilter] = React.useState("All");
    const schoolOptions = React.useMemo(() => {
        const schoolsMap = new Map();
        schoolsData.forEach(school => schoolsMap.set(school.id.toString(), school.name));
        const uniqueSchoolIds = new Set(users.map(user => user.schoolId).filter(Boolean));
    ...
    const getSchoolNameById = React.useCallback((schoolId) => {
        if (!schoolId)
            return 'N/A';
    ...
    return (<div className="space-y-8">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
            ...
            <div className="flex gap-2 flex-wrap sm:flex-nowrap">
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56">
                  ...
                  <DropdownMenuRadioGroup value={schoolFilter} onValueChange={setSchoolFilter}>
                    {schoolOptions.map(schoolId => (<DropdownMenuRadioItem key={schoolId} value={schoolId}>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuRadioGroup value={roleFilter} onValueChange={(value) => setRoleFilter(value)}>
                    {ALL_ROLES_FOR_FILTER.map(role => (<DropdownMenuRadioItem key={role} value={role}>
                        {role}
                      </DropdownMenuRadioItem>))}
                  </DropdownMenuRadioGroup>
                </DropdownMenuContent>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuRadioGroup value={statusFilter} onValueChange={(value) => setStatusFilter(value)}>
                    {ALL_STATUSES.map(status => (<DropdownMenuRadioItem key={status} value={status}>
                        {status}
                      </DropdownMenuRadioItem>))}
                  </DropdownMenuRadioGroup>
                </DropdownMenuContent>
          ...
          {filteredUsers.length === 0 ? (<p className="text-muted-foreground text-center py-4">No users match your search or filter criteria.</p>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredUsers.map((user) => (<Card key={user.id} className="flex flex-col">
                  ...
                  <CardFooter className="flex gap-2 pt-3">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent className="w-56">
                        ...
                        <DropdownMenuSeparator />
                        <DropdownMenuRadioGroup value={user.role} onValueChange={(newRole) => handleRoleChange(user.id, newRole)}>
                          {ALL_ROLES.map((roleName) => (<DropdownMenuRadioItem key={roleName} value={roleName}>
                              {roleName}
                            </DropdownMenuRadioItem>))}
                        </DropdownMenuRadioGroup>
                      </DropdownMenuContent>
                    </DropdownMenu>
                    <Button variant="outline" size="sm" className="flex-1" onClick={() => toggleUserStatus(user.id)}>
