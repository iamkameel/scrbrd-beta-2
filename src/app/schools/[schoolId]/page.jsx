...
import { ArrowLeft, Info, MapPin, Trophy, ListChecks, ClipboardList, Building, Users, BarChart, CalendarDays, FileText, Loader2, AlertTriangle } from "lucide-react";
...
const fetchSchoolFixtures = async (teamIds) => {
    ...
    const processSnapshot = (snapshot) => {
        snapshot.docs.forEach((docSnap) => {
            ...
            const fixtureDisplayData = {
                ...
                time: data.time,
                matchType: data.matchType || 'Unknown', // Attempt to get matchType, default to Unknown if not present
...
const fetchSchoolResults = async (fixtureIds) => {
...
export default function SchoolProfilePage() {
    ...
    const { data: school, isLoading: isLoadingSchool, isError: isErrorSchool, error: schoolError } = useQuery({
        queryKey: ['school', schoolId],
        queryFn: () => fetchSchool(schoolId),
        enabled: !!schoolId,
    });
    const schoolTeamIds = React.useMemo(() => school?.teams?.map(t => t.id) || [], [school]);
    const { data: schoolFixtures, isLoading: isLoadingFixtures } = useQuery({
    ...
    const { data: schoolResultsData, isLoading: isLoadingResults } = useQuery({
    ...
    return (<div className="container mx-auto py-8 space-y-8">
      ...
      {school.bannerImageUrl && (<Card className="overflow-hidden shadow-lg rounded-xl">
          <div className="relative h-48 md:h-72">
            <Image src={school.bannerImageUrl} alt={`${school.name} Banner`} fill style={{ objectFit: "cover" }} priority data-ai-hint="school building campus"/>
          </div>
        </Card>)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          ...
          {school.fields && school.fields.length > 0 && (<Card>
              <CardHeader>
                <CardTitle className="text-xl flex items-center gap-2">
              ...
              <CardContent>
                <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                  {school.fields.map((field, index) => (<li key={index}>{field}</li>))}
                </ul>
              </CardContent>
            </Card>)}
          ...
          <Card>
            ...
            <CardContent>
              {schoolResults.length > 0 ? (<div className="space-y-4">
                  {schoolResults.map((result) => {
                const fixture = schoolFixtures?.find(f => f.id === result.fixtureId);
                return (<div key={result.id} className="p-4 border rounded-lg bg-muted/50 shadow-sm space-y-2"> {/* Line before the target line */}
 ...
 {fixture && <p className="text-xs text-muted-foreground">{format(parseISO(fixture.date), 'EEE, MMM d, yyyy')}</p>}
                ...
                </div>) : (<p className="text-muted-foreground">No recent match results found for {school.name}.</p>)}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              {upcomingSchoolFixtures.length > 0 ? (<div className="space-y-4">
                ...
                return (<div key={fixture.id} className="p-4 border rounded-lg bg-muted/50 shadow-sm space-y-2">
 <div className="flex justify-between items-start">
 <p className="font-semibold text-md">{fixture.homeTeamName} vs {fixture.awayTeamName}</p>
                          <Badge variant={badgeVariant} className={cn("whitespace-nowrap", currentStatus === "Upcoming" && "bg-[hsl(var(--accent))] text-accent-foreground border-transparent", fixture.status === "Completed" && "bg-[hsl(120,60%,30%)] text-accent-foreground border-transparent", fixture.status === "Live" && "bg-destructive text-destructive-foreground border-transparent animate-pulse", fixture.status === "Rain-Delay" && "bg-[hsl(var(--primary))] text-primary-foreground border-transparent opacity-80", fixture.status === "Play Suspended" && "bg-[hsl(var(--chart-3))] text-card-foreground border-transparent", fixture.status === "Match Abandoned" && "bg-[hsl(var(--secondary))] text-muted-foreground opacity-80 border-transparent")}>
                            {currentStatus}
                          </Badge>
                        </div>
                        ...
                        {(currentStatus === "Upcoming" || currentStatus === "Scheduled") && (<Button asChild variant="outline" size="sm" className="mt-2">
                ...
                </div>) : (<p className="text-muted-foreground">No upcoming fixtures found for {school.name}.</p>)}
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          {school.teams && school.teams.length > 0 && (<Card>
              <CardHeader>
                <CardTitle className="text-xl flex items-center gap-2">
            ...
            <div key={team.id} className="flex justify-between items-center p-2 bg-muted/50 rounded-md text-sm">
                  ...
                  {school.teams.length > 10 && (<p className="text-xs text-muted-foreground text-center mt-2">
          ...
          {school.awardsAndAccolades && school.awardsAndAccolades.length > 0 && (<Card>
              <CardHeader>
                <CardTitle className="text-xl flex items-center gap-2">
              ...
              <CardContent>
                <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                  {school.awardsAndAccolades.map((award, index) => (<li key={index}>{award}</li>))}
                </ul>
              </CardContent>
            </Card>)}

          {school.records && school.records.length > 0 && (<Card>
              <CardHeader>
                <CardTitle className="text-xl flex items-center gap-2">
              ...
              <CardContent>
                <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                  {school.records.map((record, index) => (<li key={index}>{record}</li>))}
                </ul>
              </CardContent>
            </Card>)}
        </div>
