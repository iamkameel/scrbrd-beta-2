...
export default function DashboardPage() {
    ...
    return (<div className="container mx-auto py-8">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {features.map((feature) => (<Card key={feature.href} className="flex flex-col overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
            <CardHeader className="pb-4 flex-grow">
              <div className="flex items-center gap-3">
                {feature.icon && <feature.icon className="h-8 w-8 text-[hsl(var(--primary))]"/>}

                <CardTitle className="text-xl">{feature.label ?? 'Feature'}</CardTitle>
              </div>
            </CardHeader>
            <CardContent className="flex-grow">
              <CardDescription>
                Access and manage {feature.label?.toLowerCase() ?? 'this feature'}.
