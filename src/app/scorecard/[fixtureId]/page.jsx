...
import { fetchScorecardData } from '@/lib/results-data';
...
function getTopBatsmen(teamName, inningsData, count) {
    if (!inningsData)
        return [];
    const allTeamBattingScores = []; // Corrected variable name type
...
function getTopBowlers(teamName, inningsData, count) {
    if (!inningsData)
        return [];
    const allTeamBowlingScores = []; // Corrected variable name type
    inningsData.forEach((inning) => {
...
function generatePartnershipUIData(innings) {
    const partnerships = [];
    if (!innings || !innings.battingScores || innings.battingScores.length === 0) {
        return partnerships;
    }
    const battingScoresMap = new Map();
    innings.battingScores.forEach((bs) => battingScoresMap.set(bs.name, bs)); // Explicit type
    let currentScore = 0;
    let activeBatsmen = [
    ...
    for (let i = 0; i < fallOfWickets.length; i++) {
        ...
        if (activeBatsmen[0]?.name === fow.batsmanOut) {
            ...
            const otherBatsmenAtCrease = innings.battingScores.filter(bs => bs.name !== fow.batsmanOut &&
                (activeBatsmen[0]?.name === bs.name || activeBatsmen[1]?.name === bs.name));
            if (otherBatsmenAtCrease.length > 0)
                partnerBatsmanObj = otherBatsmenAtCrease[0];
        }
        partnerships.push({
...
const PartnershipTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        const partnershipData = payload[0].payload; // Explicit type
        let partnerDisplay = "";
        ...
        if (partnershipData.batsman2Name && partnershipData.batsman2Name !== "N/A" && partnershipData.batsman2Name !== "Partner") {
            if (partnerDisplay && partnershipData.batsman1Name !== partnershipData.batsman2Name)
                partnerDisplay += " & ";
        ...
        if (!partnerDisplay)
            partnerDisplay = "Partnership";
        return (<div className="bg-card text-card-foreground p-3 rounded-md shadow-md border">
...
export default function ScorecardPage() {
    ...
    const fixtureId = params.fixtureId ? params.fixtureId : null; // Keep fixtureId as string for fetching
    const [scorecardData, setScorecardData] = React.useState(null); // Updated type
        ...
        const loadScorecard = async () => {
            ...
            try {
                ...
                if (data) {
                    ...
                    setScorecardData(data); // Explicit cast if confident in data structure
                }
                else {
 ...
 <p className="text-3xl font-bold text-primary">{result.teamBScore}</p> {/* Corrected font size for consistency */}
            ...
            <CardContent className="p-0 flex justify-center"> {/* Added flex and justify-center */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8 w-full max-w-4xl"> {/* Adjusted spacing and added max-width */}
                <div className="space-y-4" id={`top-performers-${result.teamAId}`}> {/* Use teamAId */}
                  ...
                  {(topTeamABatsmen.length > 0 || topTeamABowlers.length > 0) ? (<div className="space-y-3">
                      {topTeamABatsmen.length > 0 && (<div className="space-y-1">
                          <p className="text-sm font-medium text-muted-foreground">Top Batsmen</p>
                          <ul className="space-y-1">
                            {topTeamABatsmen.map((batsman, index) => ( // Explicit types
                <li key={`tA-batsman-${index}`} className="text-sm flex justify-between items-center border-b last:border-b-0 py-1">
                      ...
                      {topTeamABowlers.length > 0 && (<div className="space-y-1 pt-2">
                          <p className="text-sm font-medium text-muted-foreground">Top Bowlers</p>
                          <ul className="space-y-1">
                            {topTeamABowlers.map((bowler, index) => ( // Explicit types
                <li key={`tA-bowler-${index}`} className="text-sm flex justify-between items-center border-b last:border-b-0 py-1">
                    ...
                    </div>) : (<p className="text-sm text-muted-foreground">No top performer data available for {result.teamAId}.</p>)} {/* Keep result.teamA for display as it seems to be the team name here */}
                </div>
                ...
                <div className="space-y-4" id={`top-performers-${result.teamBId}`}> {/* Use teamBId */}
                  ...
                  {(topTeamBBatsmen.length > 0 || topTeamBBowlers.length > 0) ? ( /* Check both batsmen and bowlers */<div className="space-y-3">
                      {topTeamBBatsmen.length > 0 && (<div className="space-y-1">
                          <p className="text-sm font-medium text-muted-foreground">Top Batsmen</p>
                          <ul className="space-y-1">
                            {topTeamBBatsmen.map((batsman, index) => ( // Explicit types
                <li key={`tB-batsman-${index}`} className="text-sm flex justify-between items-center border-b last:border-b-0 py-1">
                      ...
                      {topTeamBBowlers.length > 0 && (<div className="space-y-1 pt-2">
                          <p className="text-sm font-medium text-muted-foreground">Top Bowlers</p>
                          <ul className="space-y-1">
                            {topTeamBBowlers.map((bowler, index) => ( // Explicit types
                <li key={`tB-bowler-${index}`} className="text-sm flex justify-between items-center border-b last:border-b-0 py-1">
                    ...
                    </div>) : (<p className="text-sm text-muted-foreground">No top performer data available for {result.teamBId}.</p>)} {/* Use result.teamBId for display as it seems to be the team name here */}
                </div>
              </div>
            </CardContent>
            ...
            <Card className="mt-6">
              ...
              <CardContent className="space-y-6">
                {result.innings && result.innings.length > 0 ? (<Tabs defaultValue={defaultTabValue} className="w-full">
                    <TabsList className="grid w-full grid-cols-1 sm:grid-cols-2 md:grid-cols-none md:flex md:flex-wrap justify-start">
                      {result.innings.map((inningData) => (<TabsTrigger key={`trigger-innings-${inningData.inningsNumber}`} value={`innings-${inningData.inningsNumber}`}> {/* Ensure consistent value */}
                ...
                return (<TabsContent key={`content-innings-${inningData.inningsNumber}`} value={`innings-${inningData.inningsNumber}`} className="mt-4">
 ...
 <TableCaption>Batting scorecard for {inningData.battingTeam}</TableCaption>
                                  <Table>
                                    ...
                                    <TableBody>
 {inningData.battingScores.map((batter) => ( // Explicit type
                    <TableRow key={batter.name}>
                                ...
                                <div className="p-4 pt-0">
                                  ...
                                  {partnershipVisualData.length > 0 ? (<ChartContainer config={chartConfig} className="min-h-[300px] w-full aspect-auto">
                                      <BarChart layout="vertical" data={partnershipVisualData} margin={{ top: 5, right: 40, left: 30, bottom: 20 }} // Increased margins
                    ...
                    >
                                    ...
                                    </ChartContainer>) : (<p className="text-sm text-muted-foreground">Partnership data not available for visualization.</p>)}
                                </div>
                              </AccordionContent>
                            </AccordionItem>
 ...
 <TableCaption>Bowling figures for {inningData.bowlingTeam}</TableCaption>
                                  <Table>
                                    ...
                                    <TableBody>
 {inningData.bowlingScores.map((bowler) => ( // Explicit type
 ...
 <TableCaption>Fall of wickets for {inningData.battingTeam}</TableCaption>
                                  ...
                                  {inningData.fallOfWickets && inningData.fallOfWickets.length > 0 ? (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
 {inningData.fallOfWickets.map((fow, idx) => (<Card key={`fow-${idx}`} className="bg-card text-card-foreground shadow-sm text-sm p-3">
                                    ...
                                    </div>) : (<p className="p-4 text-sm text-muted-foreground">Fall of wickets data not available for this innings.</p>)}
                                </div>
                               </AccordionContent>
                            </AccordionItem>
                  ...
                  </Tabs>) : (<p className="text-muted-foreground">
