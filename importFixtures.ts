...
import { fixtures as sourceFixtures, type Fixture as SourceFixture } from './src/lib/fixtures-data'; // Adjust the path if necessary
...
const deriveAgeGroup = (teamAId: string, teamBId: string): string => {
  const idsToTest = [teamAId.toLowerCase(), teamBId.toLowerCase()];
  for (const id of idsToTest) {
    if (id.includes('_u19')) return 'U19';
    if (id.includes('_u16')) return 'U16';
    if (id.includes('_u15')) return 'U15';
    if (id.includes('_u14')) return 'U14';
    if (id.includes('_u13')) return 'U13';
  }
